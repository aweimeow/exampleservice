...
import oaiservice_stats as stats
import threading
import subprocess
import six
import uuid
...
publisher_id='oaiservice_publisher'

@app.route('/monitoring/agent/oaiservice/start',methods=['POST'])
def oaiservice_start_monitoring_agent():
    ...
    try:
        ...
        logging.info("OAIservice monitoring is enabled")
        return "OAIservice monitoring is enabled"
    except Exception as e:
            return e.__str__()

@app.route('/monitoring/agent/oaiservice/stop',methods=['POST'])
def openstack_stop():
    global start_publish
    start_publish = False
    logging.info ("OAIservice monitoring is stopped")
    return "OAIservice monitoring is stopped"
...
def publish_oaiservice_stats(oai_stats):
     global producer
     global keystone_tenant_id, keystone_user_id, publisher_id

     for k,v in oai_stats.iteritems():
          msg = {'event_type': 'cord.'+k,
                 ...
                 'payload': {'counter_name':k,
                             ...
                             'resource_id':'oaiservice',
                             'user_id':keystone_user_id,
                             'tenant_id':keystone_tenant_id
                            }
                }
          producer.publish(msg)
          logging.debug('Publishing oaiservice event: %s', msg)
...
def periodic_publish():
    ...
    publish_oaiservice_stats(resParse)
...
if __name__ == "__main__":
    ...
    logging.info ("OAIservice monitoring is listening on port 5004")
